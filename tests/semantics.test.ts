...
describe("findOpportunities — отбор пробелов семантики", () => {
  ...
  const queries = [
    ...
    { phrase: "боль", visits: 100 },                            // < 5 символов — отсев
  ];
...
describe("почти-дубли тем в отборе", () => {
  it("одна тема — одно предложение («как лечить » ≈ «лечение »)", () => {
    const res = findOpportunities(
      [
        { phrase: "как лечить мигрень", visits: 50 },
        { phrase: "лечение мигрени", visits: 12 },
      ],
      [{ slug: "x", title: "Гирудотерапия" }]
    );
    expect(res).toHaveLength(1);
    expect(res[0].phrase).toBe("как лечить мигрень");
  });
});
