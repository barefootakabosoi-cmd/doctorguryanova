...
export function validateGeneratedClaims(text: string, dossier: ResearchDossier): GeneratedClaimsValidation {
  ...
  if (dossierCautions.length === 0) {
    const fabricatedCaution = cleanText.match(
      /<h[2-6][^>]*>[^<]*(?:предостережени|медицинские рекомендации|меры предосторожности)|(?:необходима|нужна|стоит|требуется|следует)\s+консультац[а-яё]*|консультац[а-яё]*\s+(?:с\s+)?(?:врач|специалист)|обращ[а-яё]*\s+(?:к\s+)?(?:врач|специалист)|обратит[а-яё]*\s+(?:к\s+)?(?:врач|специалист)|рекомендуется\s+пройти|следует\s+исключительно|начин[а-яё]*\s+занятия\s+следует/i
    );
...
12. Заголовок ([TITLE] и [TG_TITLE]) обязан сохранять предмет темы и её целевую группу. Уточнение внутри группы допустимо («мигрень у женщин» → «мигрень у беременных женщин»), обобщение с потерей группы — нет («мигрень у женщин» → «мигрень у взрослых пациентов» запрещено).
13. Запрещено утверждать исходы, которых нет в разрешённых утверждениях: не приписывай улучшение исходам из списка «НЕ ИЗУЧЕНО», не придумывай уровни доказательности, врачебное наблюдение и ограничения, которых нет в досье. Раздел клинических предостережений создавай только при непустом списке предостережений досье.
...
// Model-emitted closing tags occasionally come mis-slashed ("/[TG_POST]"
// instead of "[/TG_POST]"), which makes extract() miss the closing tag: the
// fallback then swallows the stray slash, and for [CONTENT] a missing closer
