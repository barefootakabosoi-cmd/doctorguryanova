...
// Убираем эмодзи и проблемные Unicode-символы
function stripEmoji(text: string): string {
  return text.replace(/[\u{1F000}-\u{1FFFF}\u{2600}-\u{27BF}\u{2190}-\u{21FF}\u{2B00}-\u{2BFF}]/gu, "");
}
...
function slugify(text: string): string {
  ...
  return transliterate(text).replace(/[^a-z0-9\s-]/g, "").trim().replace(/\s+/g, "-").substring(0, 80);
}
...
export async function generateArticle(topic: string, cluster?: KeywordCluster): Promise<GeneratedContent> {
  ...
  console.log(`[pipeline] PubMed: ${rawPubmed.length} статей`);

  // 2. Crossref
  const crossrefArticles = await searchCrossRef(pubmedQuery, 5);
  console.log(`[pipeline] Crossref: ${crossrefArticles.length} статей`);

  // Объединяем и дедуплируем
  const allArticles = dedupeArticles([...rawPubmed, ...crossrefArticles]).slice(0, 7);
  console.log(`[pipeline] Всего уникальных: ${allArticles.length}`);

  // 3. GigaChat — статья от врача
  ...
  const systemPrompt = ` Гурьянова Валентина Андреевна, выпускница 1-го МГМУ им. Сеченова (1977), 49 лет клинической практики. Специализация: неврология, рефлексотерапия, гирудотерапия, остеопатия.

Ты пишешь медицинскую статью для блога. Читатели — пациенты и коллеги-врачи. Тон — профессиональный, экспертный, как в медицинском журнале.
...
- "уникальный метод", "эффективный метод"
- "не откладывайте визит"
- "верните себе здоровье"
- любой призыв к действию с "прямо сейчас", "сегодня", "немедленно"

Пиши как врач в медицинской статье, а не как маркетолог в рекламе.

Также ЗАПРЕЩЕНО:
- Использовать эмодзи (никаких ✅, 🔹, 📌, ▶ и т.д.)
- Использовать спецсимволы (→, ✓, ★)
- Псевдонаучные объяснения ("энергетические меридианы", "очищение организма")

Разрешено:
- Писать от первого лица: "В клинической практике...", "По моему опыту..."
- Ссылаться на исследования: "Согласно исследованию..."
- Давать практические рекомендации

СТРУКТУРА:
1. Введение — распространённость, актуальность
...
Дисклеймер в конце: "Информация носит образовательный характер и не заменяет консультацию врача."`
...
- Опирайся ТОЛЬКО на предоставленные abstract'ы — НЕ выдумывай исследования
- Если данных мало — честно пиши "данных недостаточно" или "требуются дальнейшие исследования"
- Указывай доказательную базу: "Согласно исследованию...", "В рандомизированном исследовании..."
...
Напиши профессиональную медицинскую статью. Объём: 2000-3000 слов.`;
  ...
  // 4. GigaChat — title, excerpt, keywords
...
2. DESCRIPTION (150-170 символов, информативный)
3. KEYWORDS (5-7 слов через запятую)
...
KEYWORDS: ...` },
  ...
  // 5. GigaChat — Telegram-пост
  const tgResult = await chatCompletion({
    messages: [
      { role: "system", content: "Ты — врач, ведёшь профессиональный Telegram-канал. Тон экспертный. КАТЕГОРИЧЕСКИ БЕЗ рекламы: никаких \\"запишитесь\\", \\"бесплатная консультация\\", \\"прямо сейчас\\". БЕЗ эмодзи." },
      { role: "user", content: `Напиши пост для Telegram (800-1200 символов) на основе статьи. Тема: ${topic}

Правила:
- Экспертный тон, как врач
