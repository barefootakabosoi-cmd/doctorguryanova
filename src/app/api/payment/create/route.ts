...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.TEST_MODE === "true") {
      ...
      // Генерируем Jitsi-ссылку один раз — возвращаем клиенту + шлём в каналы
      const jitsiLink = `https://meet.jit.si/guryanova-${bookingId}#config.prejoinPageEnabled=false&config.startWithAudioMuted=true`

      // Имитируем webhook через 2 сек (чтобы success-страница успела загрузиться)
      setTimeout(async () => {
        try {
          // 1. Telegram врачу
...
<i>⚠️ Это тест — деньги не списаны!</i>`
      ...
      // Возвращаем jitsiLink клиенту — BookingForm положит в localStorage для success-страницы
